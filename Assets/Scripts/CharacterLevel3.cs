...
public class CharacterLevel3 : MonoBehaviour
{
    ...
    GameObject keys;
    GameObject[] greenSlime;
    GameObject orangeSlime;

    public GameObject canvas;
    private int hitOrange = 0;
    ...
    private void Start()
    {
        ...
        greenSlime = GameObject.FindGameObjectsWithTag("GreenSlime");
        orangeSlime = GameObject.FindGameObjectWithTag("OrangeSlime");
    ...
    private void Reset()
    {
        if (gm.vidas > 0)
        {
            ...
            foreach (GameObject s in greenSlime)
            {
                s.SetActive(true);
            }
            keys.SetActive(true);
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.tag == "GreenSlime")
        {
            ...
            audioSource.PlayOneShot(slime, 0.1f);
        }

        if (collision.gameObject.tag == "OrangeSlime")
        {
            if(transform.position.y >= -16){
                print("entrei");
                if(hitOrange == 1){
        ...
        {
            audioSource.PlayOneShot(spike, 0.1f);
            gm.pontos-=5;
        }
